...
class AdvancedSettings(config: AbstractConfigView, path: List<String>) : ConfigView(config, path) {
    ...
    class LocationConfig(config: AbstractConfigView, path: List<String>) : ConfigView(config, path) {
        var xPos by defining(5)
        var yPos by defining(17)
        var width by defining(300)
        var height by defining(160)
    }
}
